...
def try_import_vowpalwabbit():
    try:
        ...
        assert vowpalwabbit_version >= parse_version("9.0.0") and vowpalwabbit_version <= parse_version(
            "9.8.0"
        ), f"Currently, we only support vowpalwabbit version >=9.0 and <=9.8. Found vowpalwabbit version: {vowpalwabbit_version}"
    except ImportError:
        raise ImportError("`import vowpalwabbit` failed.\n" "A quick tip is to install via `pip install vowpalwabbit>=9,<=9.8")
